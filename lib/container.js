...
Container.prototype.get = function (name) {
    ...
    if (wrapper.isFactory()) {
        // if any parameters additional to name were passed along then we want to call the factory and pass the parameters on...
        var parameters = Array.from(arguments);
        if (parameters.length > 1) {
            // remove the first element from the array
            parameters.splice(0, 1);
            // instantiate the factory passing the additional parameters
            // using the spread operator to the transform
            return wrapper.getValue()(container, ...parameters);
        }
