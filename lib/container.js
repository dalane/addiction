...
Container.prototype.add = function (name, dependency, tags) {
    ...
    if (tags.length == 0) {
        throw new SyntaxError('Tags array is empty.');
    }
...
Container.prototype.remove = function (name) {
    ...
    // check if the requested dependency is tagged
    if (name in this._dependency_tags) {
        // iterate through each tag and remove the reference from _tagged
        var self = this;
        this._dependency_tags[name].forEach(function (tag) {
            self._tagged[tag].forEach(function (linked_name) {
                if (name == linked_name) {
                    delete self._tagged[tag][linked_name];
                }
            });
        });
        // remove the dependency name from _dependency_tags
